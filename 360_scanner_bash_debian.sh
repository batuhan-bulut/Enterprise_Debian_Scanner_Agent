...
echo Description=acunetix.service description >> "$AGENT_NAME".service
...
echo ExecStart=/usr/bin/dotnet "$WORK_DIR"/Acunetix.Cloud.Agent.dll >> "$AGENT_NAME".service
echo ExecStop=/usr/bin/pkill -f ""$WORK_DIR"/Nhs/AcunetixHelperService.exe" >> "$AGENT_NAME".service
